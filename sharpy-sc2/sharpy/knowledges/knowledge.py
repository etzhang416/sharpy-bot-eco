...
class Knowledge:
    ...
    async def update(self, iteration: int):
        main_zone = self.expansion_zones[0]
        should_close_gate = True
        for unit in main_zone.known_enemy_units:  # type: Unit
            ...
            if unit.is_structure:
                should_close_gate = False
                break
            # Dangerous enemy near our base!
            if self.ai.get_terrain_height(unit) < main_zone.height:
        ...
        if self.close_gates and (not should_close_gate):
