...
from sharpy.managers.game_states.advantage import (
    ...
    at_least_clear_disadvantage, at_least_advantage,
)
...
class PlanZoneAttack(ActBase):
    ...
    RETREAT_POWER_PERCENTAGE = 0.7   # small -> aggressive

    def __init__(self, start_attack_power: float = 2):
    ...
    async def debug_actions(self):
        ...
        self.retreat = AttackStatus.Retreat
        if self.status == self.retreat:
    ...
    def _should_attack(self, power: ExtendedPower) -> bool:
        ...
        if zone_count == 1 and enemy_main.is_enemys:
            ...
            if (self.knowledge.enemy_race == Race.Terran
                    and (self.knowledge.enemy_units_manager.unit_count(UnitTypeId.PLANETARYFORTRESS) >= 1 or
                         self.knowledge.enemy_units_manager.unit_count(UnitTypeId.BUNKER) >= 1)):
                multiplier *= 2.2
            elif (self.knowledge.enemy_race == Race.Terran
                    and (self.knowledge.enemy_units_manager.unit_count(UnitTypeId.SIEGETANK) >= 1 or
                         self.knowledge.enemy_units_manager.unit_count(UnitTypeId.WIDOWMINE) >= 1)):
                multiplier *= 2
    ...
    def _should_retreat(self, fight_center: Point2, already_attacking: Units) -> AttackStatus:
        ...
        if self.attack_on_advantage and enemy_local_power.power > 8:
            if (
                    self.game_analyzer.our_army_predict in at_least_clear_advantage
                    and self.game_analyzer.our_income_advantage in at_least_small_disadvantage
            ) or (
            ...
            if ((self.game_analyzer.our_army_predict in at_least_small_disadvantage
                 and self.game_analyzer.our_income_advantage in at_least_clear_advantage)
                    or (self.game_analyzer.our_army_predict in at_least_clear_disadvantage
                        and self.game_analyzer.our_income_advantage in at_least_small_advantage)):
                # Our amirmy is smaller but economy is better, focus on defence!
        ...
        if self.ai.time <= 3 * 60:
            self.retreat_multiplier = 0.2
        elif self.ai.time <= 8 * 60:
            if self.knowledge.enemy_race == Race.Zerg:
                self.retreat_multiplier = 0.25
            elif self.knowledge.enemy_race == Race.Terran:
                self.retreat_multiplier = 0.6
            else:
                if self.cache.own_townhalls.amount >= 2:
                    self.retreat_multiplier = 0.9
                else:
                    self.retreat_multiplier = 0.5
        else:
            self.retreat_multiplier = PlanZoneAttack.RETREAT_POWER_PERCENTAGE
    ...
    def _get_target(self) -> Optional[Point2]:
        ...
        for zone in enemy_zones:  # type: Zone
            not_like_points = zone.center_location.distance_to(start_position) / 7
            not_like_points += zone.enemy_static_power.power * 7

            if zone.enemy_townhall is not None:
                if zone.enemy_townhall.build_progress < 1:
                    not_like_points *= 0.3
