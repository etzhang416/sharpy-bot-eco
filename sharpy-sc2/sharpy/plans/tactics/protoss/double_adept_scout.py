    ...
    async def micro_adepts(self, adepts: Units):
        ...
        for adept in adepts:  # type: Unit
            ...
            if shade_tag is not None:
                ...
                enemy_workers = self.knowledge.unit_cache.enemy_in_range(shade.position3d, 6).of_type(
                    [UnitTypeId.SCV, UnitTypeId.PROBE, UnitTypeId.DRONE, UnitTypeId.MULE]
                )
                could_harass = enemy_workers.amount >= 2
                if self.cd_manager.is_ready(adept.tag, AbilityId.ADEPTPHASESHIFT_ADEPTPHASESHIFT, 6.9):
                    ...
                    ) < self.knowledge.enemy_units_manager.danger_value(adept, shade.position) and \
                            not could_harass:
