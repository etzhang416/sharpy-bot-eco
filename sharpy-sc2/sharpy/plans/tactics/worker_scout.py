...
class WorkerScout(ActBase):
    ...
    def scout_enemy_expansions(self):
        ...
        if self.knowledge.enemy_race == Race.Terran:
            self.scout_locations = map_to_point2s_minerals(self.zone_manager.enemy_expansion_zones[1:should_scout])
        else:
            self.scout_locations = map_to_point2s_minerals(self.zone_manager.enemy_expansion_zones[0:should_scout])
    ...
    async def execute(self) -> bool:
        ...
        if self.scout is None or (self.ai.time >= 2*60 + 15 and self.knowledge.enemy_race == Race.Terran) \
                or (self.ai.time >= 2*60 + 15 and self.knowledge.enemy_race == Race.Zerg)\
                or (self.ai.time >= 2*60 + 30 and self.knowledge.enemy_race == Race.Protoss):
            # No one to scout
