...
class DistributeWorkers(ActBase):
    ...
    async def execute(self) -> bool:
        ...
        if self.max_gas is not None:
            self.gas_workers_max = self.max_gas
        ...
        if (
                self.active_gas_workers > self.gas_workers_target == 0
        ):
            for worker in self.ai.workers:
                if worker.is_carrying_vespene:
                    self.add_worker(worker, self.ai.mineral_field.closest_to(worker))
