...
class PlanZoneDefense(ActBase):
    ...
    def defense_required(self, enemies: Units):
        ...
        if enemies.exists:
            if enemies.amount <= 1 \
                    and (enemies.first.type_id == UnitTypeId.OVERLORD or enemies.first.type_id == UnitTypeId.SCV
                         or enemies.first.type_id == UnitTypeId.DRONE or enemies.first.type_id == UnitTypeId.PROBE) \
                    and self.ai.time <= 100:
                return False
            if enemies.amount <= 1 and 4 * 60 <= self.ai.time <= 8 * 60 + 30 and self.knowledge.enemy_race == Race.Zerg \
                    and enemies.first.type_id == UnitTypeId.OVERLORD:
                return False
            return True
        return False
            ...
            if zone_defenders.exists or zone.is_ours or zone == self.knowledge.own_main_zone:
                if not self.defense_required(enemies):
                    ...
                    if (
                            ...
                            or self.zone_seen_enemy[index] + PlanZoneDefense.ZONE_CLEAR_TIMEOUT < self.ai.time
                    ):
                ...
                for unit in self.knowledge.roles.units(UnitTask.Idle):
    ...
    async def worker_defence(
        ...
        if self.ai.time < 3 * 60 and self.knowledge.enemy_race == Race.Zerg:
            # efficiently use wall and keep mining
            defense_count_panic = defense_count_panic * 0.4
