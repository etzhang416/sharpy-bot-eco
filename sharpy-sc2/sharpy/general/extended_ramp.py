...
class RampPosition(enum.Enum):
    ...
    PylonBlockVsTerran = 13
    GateBlockVsTerran = 14
    CoreBlockVsTerran = 15
...
class ExtendedRamp:
    ...
    def __init__(self, ramp: Ramp, ai: sc2.BotAI):
        self.start_location = ai.start_location
        self.map_name = ai.game_info.map_name
        self.ramp = ramp
        # Do NOT Modify
    ...
    def find_ultimatum(self, ai: sc2.BotAI):
        ...
        ############################################################## no use
        # maps available for reaper block:
        # SubmarineLE
        # PillarsofGoldLE
        # EverDreamLE

        if self.map_name == "Submarine LE":
            if self.start_location.x == 135.5:
                # bottom right spawn location
                self.positions[RampPosition.PylonBlockVsTerran] = Point2((125.5, 44))
                self.positions[RampPosition.GateBlockVsTerran] = Point2((120, 42))
                self.positions[RampPosition.CoreBlockVsTerran] = self.positions[RampPosition.GateInner]
            else:
                # top left spawn location
                self.positions[RampPosition.PylonBlockVsTerran] = Point2((43, 119))
                self.positions[RampPosition.GateBlockVsTerran] = Point2((47, 120))
                self.positions[RampPosition.CoreBlockVsTerran] = self.positions[RampPosition.GateInner]

        elif self.map_name == "Pillars of Gold LE":
            if self.start_location.x == 137.5:
                # top right spawn location
                self.positions[RampPosition.PylonBlockVsTerran] = Point2((123, 122))
                self.positions[RampPosition.GateBlockVsTerran] = Point2((121.5, 124.5))
                self.positions[RampPosition.CoreBlockVsTerran] = Point2((120, 127))
            else:
                # bottom left spawn location
                self.positions[RampPosition.PylonBlockVsTerran] = Point2((45, 50))
                self.positions[RampPosition.GateBlockVsTerran] = Point2((46, 47.5))
                self.positions[RampPosition.CoreBlockVsTerran] = Point2((47, 44.5))

        elif self.map_name == "Ever Dream LE":
            if self.start_location.x == 139.5:
                # top right spawn location
                self.positions[RampPosition.PylonBlockVsTerran] = Point2((134, 145.5))
                self.positions[RampPosition.GateBlockVsTerran] = Point2((136, 145.5))
                self.positions[RampPosition.CoreBlockVsTerran] = Point2((139, 145.5))
            else:
                # bottom left spawn location
                self.positions[RampPosition.PylonBlockVsTerran] = Point2((66, 67))
                self.positions[RampPosition.GateBlockVsTerran] = Point2((63.5, 66.5))
                self.positions[RampPosition.CoreBlockVsTerran] = Point2((60.5, 66.5))

        else:
            # other maps can't block reaper, too much cliff area
            self.positions[RampPosition.PylonBlockVsTerran] = self.positions[RampPosition.Between]
            self.positions[RampPosition.GateBlockVsTerran] = self.positions[RampPosition.GateInner]
            self.positions[RampPosition.CoreBlockVsTerran] = self.positions[RampPosition.CoreInner]
##############################################################
