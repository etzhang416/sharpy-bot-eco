...
class CombatModel:
    ...
    Escort = 5  # group has kite units like colossi/tempest and we just keep distance and escort
...
class GenericMicro(MicroStep):
    ...
    def group_solve_combat(self, units: Units, current_command: Action) -> Action:
        ...
        siege_percentage_ratio = 0.2
        if self.ai.time >= 6 * 60:
            siege_percentage_ratio = 0.4

        if self.closest_group and (self.engage_ratio > 0.2 or self.can_engage_ratio > 0.2):
            # in combat
            if self.engaged_power.siege_percentage >= siege_percentage_ratio:
                self.model = CombatModel.StalkerToSiege
            elif self.engaged_power.surround_percentage > siege_percentage_ratio and not flyers:
    ...
    def unit_solve_combat(self, unit: Unit, current_command: Action) -> Action:
        ...
        if self.is_locked_on(unit) and self.enemies_near_by:
        ...
        if self.model == CombatModel.StalkerToSiege:
            if self.move_type in{MoveType.Assault, MoveType.SearchAndDestroy, MoveType.Push, MoveType.Harass}:
                siege_tanks = self.enemies_near_by.of_type(UnitTypeId.SIEGETANKSIEGED)
                if siege_tanks:
                    target = siege_tanks.closest_to(unit)
