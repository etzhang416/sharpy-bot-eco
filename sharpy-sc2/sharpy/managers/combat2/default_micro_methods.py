...
class DefaultMicroMethods:
    ...
    def focus_fire(
        ...
        lookup = min(air_range + 6, ground_range + 6)
        ...
        if prio:
            value_func = (
                lambda u: 1 if u.type_id in changelings
                else prio.get(u.type_id, -1) * (1 - u.shield_health_percentage*0.8)
            )
        else:
            value_func = (
                lambda u: 1
                if u.type_id in changelings
                else 2 * step.unit_values.power_by_type(u.type_id, 1 - u.shield_health_percentage*0.8)
            )
        ...
        for enemy in enemies:  # type: Unit
            ...
            if enemy.tag == last_target:
                score += 2
            if enemy.is_structure and not enemy.can_attack:
                score -= 6
