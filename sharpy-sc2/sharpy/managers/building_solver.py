...
class BuildingSolver(ManagerBase):
    ...
    async def solve_grid(self):
        ...
        elif self.wall_type == WallType.NoWall:
            self.terran_wall()
    ...
    def terran_wall(self):
        ramp: "ExtendedRamp" = self.knowledge.base_ramp
        if ramp and ramp.positions:
            pylon = ramp.positions.get(RampPosition.PylonBlockVsTerran)
            zealot = ramp.positions.get(RampPosition.GateZealot)
            gate = ramp.positions.get(RampPosition.GateBlockVsTerran)
            core = ramp.positions.get(RampPosition.CoreBlockVsTerran)
            self.wall_save(pylon, zealot, [gate, core])
